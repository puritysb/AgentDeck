...
**적대적 리뷰 2라운드.** 1라운드 6건 중 셋이 이번 변경이 만든 것이었다. 특히 두 데몬의
`holdsCompleteJsonObject` 가 **서로 다른 텍스트를 꺼내고 있었다** — Swift 는 균형 스캐너,
Node 는 첫 `{`부터 마지막 `}`까지의 greedy 정규식. 잘린 본문을 받아들이기 시작한 바로 그
커밋에서 이 차이가 하중을 받는다: "닫는 중괄호 뒤의 텍스트"가 정확히 그 경로이기 때문이다.
Node 도 균형 스캔을 먼저 쓰되, 키의 여는 따옴표가 빠진 본문은 문자열 추적이 어긋나므로
(그래서 `repairJudgeJson` 이 있다) greedy 구간을 두 번째 후보로 남겼다. `api` 레그의
`judgeModelLabel` 은 상수 `api:claude-opus-4-6` 이라 실행되지도 않은 모델을 eval 행의
출처로 찍고 있었고, Node 도 반대편에서 같은 결함이었다(`api:${cfg.model}` 이지만 실제로는
대체될 수 있음).

검증: Vitest 260파일 4,085 통과. macOS XCTest 740, 실패 0. 균형 스캐너는 뮤테이션으로
확인(되돌리면 3건 실패). 실제 백로그를 이 코드로 돌려 158 → 105 까지 배출했고, 독 task 는
두 번 실패 후 park 되고 **건너뛰어졌다** — 수정 전이라면 거기서 영구히 멈춘 지점이다.
