...
- **Discovery** (`bridge/src/mdns-discover.ts`): the consume side of `mdns.ts` — browses `_agentdeck._tcp`, reads TXT (`port`/`token`/`ip`), rejects link-local/loopback, confirms each candidate via `/health` (prefers the `/health` `pairingToken`, keeps only `sameSocketControl` daemons). Mirrors the iOS/Android `BridgeDiscovery`.
- **Upward auth (session → daemon)**: the session presents the daemon's advertised token — exactly how the iOS app authenticates. `DaemonWsClient` connects `ws://<host>:<port>?token=<token>` for remote, `ws://127.0.0.1:<port>` for local. The URL (which can carry the token) is never logged — only host:port plus a token yes/no marker.
