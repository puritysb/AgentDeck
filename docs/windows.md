...
- **Plugin E1 Volume dial works on Windows** — a persistent PowerShell coprocess drives CoreAudio (`IAudioEndpointVolume`); if PowerShell `Add-Type` is blocked (Constrained Language Mode / WDAC) the dial shows an N/A face. The E4 Launcher and offline fallbacks open apps/URLs via `start` / `rundll32`. macOS keeps its richer existing-tab focus behavior.
