...
| Attention context panel (experimental, default off) | Yes | Read source only | Opt-in macOS menu-bar Activity replacement over the existing roster. Local selection and last-observed context only; no agent commands, telemetry, subprocesses, entitlements or companion-install dependency. Existing Terrarium and controls are unchanged. The repository-only native observation host shares the view and reads an existing daemon; it is not bundled with or required by the App Store app. |
