...
| Cross-session coordination evidence (`RelationEvent` + `SessionInfo.coordination`) | Partial | Yes | Work divided WITHOUT a SubagentStart: `claude -p` workers spawned from a background Bash (`spawned`, process ancestry), peer messages over SendMessage (`messaged`, the sender's tool input and the receiver's `<cross-session-message>` envelope), and background jobs a session is waiting on (`waiting_on`, argv names its scratchpad). Tier 2 produces all three plus the live `coordination` census on `sessions_list` (`bridge/src/coordination-evidence.ts`). Tier 1 produces only `messaged` (hook-carried evidence) and reads every kind back for the lens; ancestry and background jobs need a process table the sandboxed daemon does not have. Observed only — never inferred from shared project membership. A `TaskCompleted` checklist item is an `info` annotation on both tiers, never a child agent. |
