...
| Hermes lifecycle monitoring | No | Yes | The external Node daemon installs an opt-in, read-only Hermes plugin that reports lifecycle events over loopback. No session control, database access, or subprocess path is added to the App Store app |
