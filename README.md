...
`--weight <n>` (any integer, default `0`) to pin a session's slot: sessions sort
by **weight ascending first** (negatives, then unweighted/`0`, then positives),
and only fall back to the default ordering *within* the same weight. Nothing
changes when no session sets a weight.
