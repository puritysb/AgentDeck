...
**Session sort weight**: session commands (`claude`/`codex`/`opencode`/`monitor`) accept `--weight <n>` (integer, default 0) — an explicit deck/tab sort override. `sortSessions` (SSOT `shared/src/session-utils.ts`, hand-mirrored in Swift `DashboardDataRules`, Kotlin `EinkFormatUtils`) orders by **weight ascending first** (negatives → unweighted/0 → positives), then the existing agentType→project→startedAt→id keys. Threads CLI → `SessionOptions.weight` → `SessionEntry.weight` (registry) → `SessionInfo.weight` (wire) → every surface. Lets a user pin Windows Terminal / iTerm tab order onto the Stream Deck. Leaving all weights at 0 reproduces the pre-weight ordering. See [README → Pinning session order](README.md#pinning-session-order-with---weight).
