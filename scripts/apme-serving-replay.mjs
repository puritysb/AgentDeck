...
const output = join(root, backend === 'ollama' ? 'ollama-no-thinking' : backend);
...
for (const selectedCase of selected) {
  for (let rep = 1; rep <= (backend === 'capture' ? 1 : 3); rep++) {
    ...
    if (backend !== 'capture' && status === false) throw new Error('Judge failure checkpointed; inspect before resuming');
  }
}
