...
Root [`VERSION`](VERSION) is the repository baseline and compatibility-line anchor, not a patch ceiling and not a runtime negotiation value. It currently reads **1.0.2**, so every maintained target must remain on compatibility line **1.0**. npm/CLI is at `1.0.21`; Stream Deck is at `1.0.6`; Android is at `1.0.10`; Apple is at `1.0.7`; ESP32 is at `1.0.5`; Ulanzi is at `1.0.3`. A target may also advance beyond root's patch without forcing unrelated targets or root to ship. **This sentence is a mirror, not a source** — take the declared values from `pnpm verify-version`, which reads each target's own manifest, and distinguish them from delivered state. Delivered state as of 2026-08-19, after the round above was cut and submitted. **Delivered:** npm `1.0.21` (all four packages readable on the registry) and ESP32 `1.0.5` (GitHub Release, 31 assets). **Submitted, awaiting review:** Apple `1.0.7` build 5201 on both platforms, Google Play `1.0.10` (versionCode 12), and Elgato Stream Deck `1.0.6`. **Still live while those are in review:** Apple `1.0.6` build 5101, Play `1.0.9` (versionCode 11), Stream Deck `1.0.5`. **Not cut:** Ulanzi stays at `1.0.3`, still under Marketplace review with a zero-commit gap.
